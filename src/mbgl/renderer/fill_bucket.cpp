...
void FillBucket::drawElements(PlainShader& shader,
    ...
    for (auto& group : triangleGroups) {
        assert(group);
        group->array[overdraw ? 1 : 0].bind(shader, vertexBuffer, triangleElementsBuffer,
                                            vertex_index, store, config);
...
void FillBucket::drawElements(PatternShader& shader,
    ...
    for (auto& group : triangleGroups) {
        assert(group);
        group->array[overdraw ? 3 : 2].bind(shader, vertexBuffer, triangleElementsBuffer,
                                            vertex_index, store, config);
        MBGL_CHECK_ERROR(glDrawElements(GL_TRIANGLES, group->elements_length * 3, GL_UNSIGNED_SHORT,
                                        elements_index));
        vertex_index += group->vertex_length * vertexBuffer.itemSize;
...
void FillBucket::drawVertices(OutlineShader& shader,
    ...
    for (auto& group : lineGroups) {
        assert(group);
        group->array[overdraw ? 1 : 0].bind(shader, vertexBuffer, lineElementsBuffer, vertex_index,
                                            store, config);
...
void FillBucket::drawVertices(OutlinePatternShader& shader,
    ...
    for (auto& group : lineGroups) {
        assert(group);
        group->array[overdraw ? 3 : 2].bind(shader, vertexBuffer, lineElementsBuffer, vertex_index,
                                            store, config);
        MBGL_CHECK_ERROR(glDrawElements(GL_LINES, group->elements_length * 2, GL_UNSIGNED_SHORT,
                                        elements_index));
        vertex_index += group->vertex_length * vertexBuffer.itemSize;
